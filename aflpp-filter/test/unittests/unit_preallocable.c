...
static void test_prealloc_overflow(void **state) {
  ...
  for (i = 0; i < PREALLOCED_BUF_SIZE + 10; i++) {
    PRE_ALLOC(prealloced[i], prealloc_me_buf, PREALLOCED_BUF_SIZE,
              prealloc_me_size);
    assert_non_null(prealloced[i]);
  }
  assert_int_equal(prealloced[0]->pre_status, PRE_STATUS_USED);
  assert_int_equal(prealloced[PREALLOCED_BUF_SIZE]->pre_status,
                   PRE_STATUS_MALLOC);

  PRE_FREE(prealloced[20], prealloc_me_size);
  PRE_ALLOC(prealloced[20], prealloc_me_buf, PREALLOCED_BUF_SIZE,
            prealloc_me_size);
  ...
  PRE_ALLOC(prealloced[PREALLOCED_BUF_SIZE], prealloc_me_buf,
            PREALLOCED_BUF_SIZE, prealloc_me_size);
  assert_non_null(prealloced[PREALLOCED_BUF_SIZE]);
  /* there should be space now! */
  assert_int_equal(prealloced[PREALLOCED_BUF_SIZE]->pre_status,
                   PRE_STATUS_USED);

  PRE_ALLOC(prealloced[0], prealloc_me_buf, PREALLOCED_BUF_SIZE,
            prealloc_me_size);
