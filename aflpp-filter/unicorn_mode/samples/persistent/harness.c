/*
   ...
   Any crashes during emulation will automatically be handled by the afl-fuzz()
   function.
...
static void mem_map_checked(uc_engine *uc, uint64_t addr, size_t size,
  ...
  if (err != UC_ERR_OK) {
    printf("Error mapping %zu bytes at 0x%llx: %s (mode: %d)\n", size,
           (unsigned long long)addr, uc_strerror(err), mode);
    exit(1);
  }
}
...
int main(int argc, char **argv, char **envp) {
  ...
  // 0x10016 argv[1][0], ...          <-^ contains the acutal input data.
