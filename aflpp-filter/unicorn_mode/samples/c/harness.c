...
// Location where the input will be placed (make sure the emulated program knows
// this somehow, too ;) )
...
static bool place_input_callback(uc_engine *uc, char *input, size_t input_len,
...
static void mem_map_checked(uc_engine *uc, uint64_t addr, size_t size,
                            uint32_t mode) {
  size = pad(size);
  ...
  if (err != UC_ERR_OK) {
    printf("Error mapping %ld bytes at 0x%llx: %s (mode: %d)\n", size,
           (unsigned long long)addr, uc_strerror(err), (int)mode);
    exit(1);
  }
}
...
int main(int argc, char **argv, char **envp) {
  if (argc == 1) {
    printf(
        "Test harness for simple_target.bin. Usage: harness [-t] "
        "<inputfile>\n");
    exit(1);
  }
  bool  tracing = false;
  ...
  uc_reg_write(uc, UC_X86_REG_RIP,
               &start_address);  // address of entry point of main()

  // Setup the Stack
  mem_map_checked(uc, STACK_ADDRESS - STACK_SIZE, STACK_SIZE,
  ...
  // 0x10016 argv[1][0], ...          <-^ contains the actual input data.
  // (INPUT_LOCATION + INPUT_OFFSET)

  uc_mem_write(uc, 0x10008, "\x16\x00\x01",
               3);  // little endian of 0x10016, see above

  // If we want tracing output, set the callbacks here
  if (tracing) {
    // tracing all basic blocks with customized callback
    uc_hook_add(uc, &hooks[0], UC_HOOK_BLOCK, hook_block, NULL, 1, 0);
    uc_hook_add(uc, &hooks[1], UC_HOOK_CODE, hook_code, NULL, BASE_ADDRESS,
                BASE_ADDRESS + len - 1);
  }
  ...
  uc_hook strlen_hook;
  uc_hook_add(uc, &strlen_hook, UC_HOOK_CODE, hook_strlen, NULL,
              strlen_hook_pos, strlen_hook_pos);

  // For persistent-iters=1, we don't need to reset this as it's
  // restarted/reforked for each run.
  uc_reg_write(uc, UC_X86_REG_RIP,
               &CODE_ADDRESS);  // Set the instruction pointer back
  // Set up the function parameters accordingly RSI, RDI (see calling
  // convention/disassembly)
  ...
  uc_afl_ret afl_ret = uc_afl_fuzz(
      ...
      filename,  // Filename of the input to process. In AFL this is usually the
                 // '@@' placeholder, outside it's any input file.
      place_input_callback,  // Callback that places the input (automatically
                             // loaded from the file at filename) in the
                             // unicorninstance
      &end_address,          // Where to exit (this is an array)
      1,                     // Count of end addresses
      NULL,                  // Optional calback to run after each exec
      false,  // true, if the optional callback should be run also for
              // non-crashes
