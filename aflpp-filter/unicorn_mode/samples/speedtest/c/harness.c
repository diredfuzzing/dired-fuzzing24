...
static void mem_map_checked(uc_engine *uc, uint64_t addr, size_t size,
  ...
  if (err != UC_ERR_OK) {
    printf("Error mapping %ld bytes at 0x%llx: %s (mode: %d)\n",
           (unsigned long)size, (unsigned long long)addr, uc_strerror(err),
           (int)mode);
    exit(1);
  }
}
...
static void hook_magicfn(uc_engine *uc, uint64_t address, uint32_t size,
                         void *user_data) {
  address += size;
...
// We use a very simple malloc/free stub here, that only works for exactly one
// allocation at a time.
static void hook_malloc(uc_engine *uc, uint64_t address, uint32_t size,
                        void *user_data) {
...
static void hook_free(uc_engine *uc, uint64_t address, uint32_t size,
                      void *user_data) {
  if (!already_allocated) {
...
int main(int argc, char **argv, char **envp) {
  if (argc == 1) {
    printf(
        "Test harness to measure speed against Rust and python. Usage: harness "
        "[-t] <inputfile>\n");
    exit(1);
  }
  bool  tracing = false;
  ...
  if (!f) {
    perror("fopen");
    puts(
        "Could not read offset to main function, make sure you ran "
        "./get_offsets.py");
    exit(-1);
  }
  ...
  // Setup the stack pointer, but allocate two pointers for the pointers to
  // input
  ...
  ssize_t exit_count =
      read_all_addrs("../target.offsets.main_ends", exits, exit_count_max);
  if (exit_count < 1) {
