...
std::string ProtoToData(const TEST &test_proto) {
  ...
  if (bb.size() != 0) { all.write(bb.c_str(), bb.size()); }
...
extern "C" MyMutator *afl_custom_init(void *afl, unsigned int seed) {
  ...
  mutator->RegisterPostProcessor(
      ...
      [](google::protobuf::Message *message, unsigned int seed) {
        ...
        // Even a dumb fuzz like `TEST.a = rand();` is better in this case...
        // Q_Q We register a post processor to apply our dumb fuzz
...
extern "C" size_t afl_custom_fuzz(
    MyMutator *mutator,                     // return value from afl_custom_init
  ...
  size_t mutated_size =
      s.size() <= max_size
          ? s.size()
          : max_size;  // check if raw data's size is larger than max_size

  delete[] mutator->mutated_out;
  mutator->mutated_out = new uint8_t[mutated_size];
  memcpy(mutator->mutated_out, s.c_str(),
         mutated_size);  // copy the mutated data
