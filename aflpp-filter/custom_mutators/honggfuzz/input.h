  ...
  (*dfunc)();
    ...
    #define defer                                        \
      void (^_STRMERGE(__defer_f_, __COUNTER__))(void)   \
          __attribute__((cleanup(__clang_cleanup_func))) \
          __attribute__((unused)) = ^

  #else /* __has_extension(blocks) */
    #define defer \
      UNIMPLEMENTED - NO - SUPPORT - FOR - BLOCKS - IN - YOUR - CLANG - ENABLED
  #endif /*  __has_extension(blocks) */
#else    /* !__clang__, e.g.: gcc */

  #define __block
  #define _DEFER(a, count)                                                \
    auto void _STRMERGE(__defer_f_,                                       \
                        count)(void *_defer_arg __attribute__((unused))); \
    int       _STRMERGE(__defer_var_, count)                              \
        __attribute__((cleanup(_STRMERGE(__defer_f_, count))))            \
        __attribute__((unused));                                          \
...
static inline uint64_t util_rnd64() {
  return rand_below(afl_struct, 1 << 30);
}

static inline const uint8_t *input_getRandomInputAsBuf(run_t  *run,
                                                       size_t *len) {
