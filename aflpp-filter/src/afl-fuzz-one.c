...
//    if (common_fuzz_stuff(afl, out_buf, temp_len)) { goto abandon_entry; }
    ...
    // @Adian: No inputs will generate at early stages if simply skip.
    if(afl->mutate_sum < afl->ndm_min) 
      afl->mutate_sum = afl->ndm_min; 
...
//    fprintf(afl->log_file,"ndm:%u,cur_val:%u\n",afl->mutate_sum,afl->stage_cur_val);
    ...
    //     afl->mutate_sum > afl->ndm_max) { goto abandon_entry; }
    // @Adian: Avoid fuzzing a seed with a mutation_sum larger than ndm_max.
    if (afl->mutate_sum > afl->ndm_max || 
        common_fuzz_stuff(afl, out_buf, temp_len)) { goto abandon_entry; }
