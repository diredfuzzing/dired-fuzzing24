...
#define SP20 SP10 SP10
  ...
  // LS :add sample display
  SAYF(SET_G1 bSTG bVR bH cCYA                   bSTOP
       " fuzzing sample " bSTG bH10 bH5 bH20 bVL cRST "\n");
  if (afl->in_sample_time) {
    SAYF(cGRA bV bSTOP "sample status : " cRST
                       "sampling for %-3ss,%-3ss last    " SP5 bSTG bV "\n",
         u_stringify_int(IB(0), afl->relative_time - afl->start_sample_time),
         u_stringify_int(IB(1), afl->end_sample_time - afl->relative_time));
  } else {
    SAYF(cGRA bV bSTOP "sample status : " cRST "paused     " SP20 SP5 bSTG bV
                       "\n");
  }
  SAYF(bV bSTOP "   start time :" cRST
                " %-2s hrs, %-2s mins, %-2s sec   " SP10 bSTG bV bSTOP
                "\n" bSTG bV bSTOP "     end time :" cRST
                " %-2s hrs, %-2s mins, %-2s sec   " SP10 bSTG bV "\n",
       u_stringify_int(IB(0), afl->start_sample_time / 3600),
       u_stringify_int(IB(1), afl->start_sample_time / 60 % 60),
       u_stringify_int(IB(2), afl->start_sample_time % 60),
       u_stringify_int(IB(3), afl->end_sample_time / 3600),
       u_stringify_int(IB(4), afl->end_sample_time / 60 % 60),
       u_stringify_int(IB(5), afl->end_sample_time % 60));
  SAYF(bV bSTOP " stored files : " cRST "%-6s" bSTG SP20 SP10 bV "\n",
       u_stringify_int(IB(0), afl->stored_num));
  SAYF(bV bSTOP "sample cycles : " cRST "%-6s" bSTG SP20 SP10 bV "\n",
       u_stringify_int(IB(0), afl->sample_num));
  //
  SAYF(SET_G1 bSTG bLB bH cCYA bSTOP " strategy:" cPIN
